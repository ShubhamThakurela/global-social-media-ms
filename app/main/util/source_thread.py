# from ..service.linkedin_service import fetch_by_file,fetch_by_url
# from ..service.linkedin_service import data_to_file, zipdir
...
#
#
# def run(source, file_path, barrier, out_path):
#     global df2
#     data_set = fetch_by_file(file_path, source)
#     df = pd.DataFrame.from_dict(data_set)
#     df2 = df2.append(df, ignore_index=True)
#     # barrier.wait()
#     df2.drop_duplicates(keep=False)
#     data_to_file(df2, "source_scrapped_data_out", out_path)
#
#
# def execute(file_path, out_path, source):
#     try:
#         start_time = time.time()
#         thread_count = 2
#         url_list = fetch_by_url(file_path, source)
#         if len(url_list) == 0:
#             return ""
#         if len(url_list) < 100:
#             thread_count = 2
